...
// genre(메인+보조 장르 조합) 문자열과 성별을 받아 캐스팅 카드 배경 이미지 URL을 반환한다. (이슈 #89)
//
...
//
// 실제 이미지가 아직 다 준비되지 않아, 지금은 placeholder URL을 사용한다.
// 이미지가 준비되는 대로 COMBO_IMAGES에 값만 추가/교체하면 된다.
public class CastingImageResolver {

    // TODO: 실제 이미지 완성 후 실제 S3 URL로 교체 (이슈 #89)
    private static final String BASE_URL = "https://placeholder-bucket.s3.ap-northeast-2.amazonaws.com/casting-images/";
    private static final String DEFAULT_FEMALE_KEY = "default-female.png";
    private static final String DEFAULT_MALE_KEY = "default-male.png";
    ...
    private static void register(String comboGenre, String mainGenre, String femaleKey, String maleKey) {
        COMBO_IMAGES.put(comboGenre, new ComboImage(mainGenre, femaleKey, maleKey));
    }

    private CastingImageResolver() {
    }

    public static String resolveImageUrl(String genre, User.Gender gender) {
        ...
        if (exactMatch != null) {
            return BASE_URL + (isMale ? exactMatch.maleKey() : exactMatch.femaleKey());
        }
        ...
        if (mainGenre != null) {
            for (ComboImage combo : COMBO_IMAGES.values()) {
                if (combo.mainGenre().equals(mainGenre)) {
                    return BASE_URL + (isMale ? combo.maleKey() : combo.femaleKey());
                }
            }
        }

        // 3) 그마저도 없으면 최종 기본 이미지
        return BASE_URL + (isMale ? DEFAULT_MALE_KEY : DEFAULT_FEMALE_KEY);
    }
